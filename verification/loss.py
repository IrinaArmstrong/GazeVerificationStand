...
class PrototypicalLoss(nn.Module):
    ...
    def forward(self, input: torch.Tensor, target: torch.Tensor, n_support: int):
        return prototypical_loss(input, target, n_support, self.__device)


def prototypical_loss(input: torch.Tensor, target: torch.Tensor,
                      n_support: int, device: Union[torch.device, str]):
    """
    ...
    query_samples = input.to(device)[query_idxs]  # get embeddings of query samples
