...
class TestFeatureGeneration(unittest.TestCase):
    ...
    def test_normalize_dataset(self):
        ...
        # Case #1
        location = dict(read_json(config.get('GazeVerification', 'verification_params')
                                  )).get('pretrained_model_location')
        location = self._current_base_path / location

        norm_data = normalize_gaze(data, to_restore=False, to_save=True, checkpoint_dir=location)
        self.assertListEqual(["data", "data_scaled"], list(norm_data.columns))

        # Case #2
        norm_data = normalize_gaze(data, to_restore=True, to_save=False, checkpoint_dir=location)
        self.assertListEqual(["data", "data_scaled"], list(norm_data.columns))
